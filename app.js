...
function renderDebtSchedule(){
  ...
  hr.innerHTML='<th>Section</th><th>Line item</th>'+MODEL_DATA.metadata.periodHeaderCells.map(k=>`<th>${format(model.get(k),'date')}</th>`).join('');
  head.appendChild(hr);
  const body=$('#debtBody'); body.innerHTML='';
  MODEL_DATA.metadata.debtScheduleRows.forEach(row=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${row.section}</td><th>${row.label}</th>`+row.cells.map(k=>`<td>${format(model.get(k),'money0')}</td>`).join('');
    body.appendChild(tr);
  });
}
function renderCharts(){
  const years=MODEL_DATA.metadata.dashboardHeaderCells.map(k=>format(model.get(k)));
