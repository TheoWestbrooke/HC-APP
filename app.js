...
function render(){
  ...
  renderCards(); renderReturns(); renderDashboardTable(); renderInputs(); renderDebtSchedule(); renderCharts();
}
function renderCards(){
  const host=$('#kpiCards'); host.innerHTML='';
  MODEL_DATA.metadata.keyCards.forEach(card=>{
    const div=document.createElement('article'); div.className='card kpi';
    div.innerHTML=`<div class="kpi-label">${card.label}</div><div class="kpi-value">${format(model.get(card.key), card.format)}</div>`;
...
function renderReturns(){
  ...
  MODEL_DATA.metadata.outputs.forEach(row=>{
    ...
    tr.innerHTML=`<th>${row.label}</th><td>${row.irr?format(model.get(row.irr),'percent1'):'–'}</td><td>${row.mom?format(model.get(row.mom),'x2'):'–'}</td>`;
...
function renderInputs(){
  const host=$('#inputSections'); host.innerHTML='';
  MODEL_DATA.metadata.inputs.forEach((section,idx)=>{
    const wrap=document.createElement('details'); wrap.className='input-section'; wrap.open = idx<2;
...
function renderComputedOnly(){
  $('#projectTitle').textContent = model.get('Capital_Inputs!D7') || 'Debt Model';
  $('#updatedDate').textContent = 'Entry date: '+format(model.get('Capital_Inputs!F14'));
  renderCards(); renderReturns(); renderDashboardTable(); renderDebtSchedule(); renderCharts();
}
function debtRowIsVisible(row){
  if(!row.condition) return true;
  const v=model.get(row.condition);
  if(typeof v === 'boolean') return v;
  return Math.abs(toNumber(v)) > 1e-9;
}
function debtRowValueFormat(row){
  const label=String(row.label || '').toLowerCase();
  if(label.includes('sculpted') || label.includes('mechanism')) return 'plain';
  return 'money0';
}
function renderDebtSchedule(){
  const head=$('#debtHead'); head.innerHTML='';
  const hr=document.createElement('tr');
  hr.innerHTML='<th>Schedule</th><th>Line item</th>'+MODEL_DATA.metadata.periodHeaderCells.map(k=>`<th>${format(model.get(k),'date')}</th>`).join('');
  head.appendChild(hr);
  const body=$('#debtBody'); body.innerHTML='';
  let lastGroup='';
  MODEL_DATA.metadata.debtScheduleRows.filter(debtRowIsVisible).forEach(row=>{
    const group=row.group || row.section || '';
    if(group !== lastGroup){
      const gr=document.createElement('tr'); gr.className='group-row';
      gr.innerHTML=`<th colspan="${2 + MODEL_DATA.metadata.periodHeaderCells.length}">${escapeHtml(group)}</th>`;
      body.appendChild(gr); lastGroup=group;
    }
    const fmt=debtRowValueFormat(row);
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${row.section}</td><th>${row.label}</th>`+row.cells.map(k=>`<td>${fmt==='plain'?escapeHtml(format(model.get(k))):format(model.get(k), fmt)}</td>`).join('');
    body.appendChild(tr);
  });
}
...
function shortDateLabel(value){
  ...
  if(d instanceof Date && !isNaN(d.getTime())) return d.toLocaleDateString(undefined,{month:'short',year:'2-digit'});
  return format(value,'date');
}
function renderCharts(){
  const years=MODEL_DATA.metadata.dashboardHeaderCells.map(k=>shortDateLabel(model.get(k)));
...
function collectOutputCacheKeys(){
  ...
  (MODEL_DATA.metadata.debtScheduleRows || []).forEach(row=>(row.cells || []).forEach(add));
...
function bindNav(){
  ...
  $('#resetBtn').addEventListener('click',()=>{ localStorage.removeItem('hcDebtModelState'); history.replaceState(null,'',location.pathname+location.search); model.reset(); render(); toast('Reset to workbook defaults'); });
